...
export class WeatherService {
  ...
  constructor() {
    this.apiKey = process.env.QWEATHER_API_KEY || '6b95a713b2854ca0b5b62ac9d9cca3bb';
    this.baseUrl = process.env.QWEATHER_BASE_URL || 'https://nd2r6wuqd7.re.qweatherapi.com';
    ...
    this.httpClient.interceptors.response.use(
      (response) => {
        if (response.data.code !== '200') {
          throw new Error(`API Error: ${response.data.code} - ${this.getErrorMessage(response.data.code)}`);
        }
        return response;
      },
      (error) => {
